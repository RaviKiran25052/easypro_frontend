...
const Writers = () => {
	...
	const getChangedFields = () => {
		...
		Object.keys(formData).forEach(key => {
			if (key === 'profilePic') {
				// For profilePic, only include if it's a File object (new upload)
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (isEdit) {
				// Edit mode - use PATCH with only changed fields
				...
				// Add mode - use POST with all fields
				const formDataToSend = new FormData();
	...
	return (
		<>
			<NavBar />
			<div className="min-h-screen px-4 md:px-20 py-6">
												...
												<div key={writer._id} className="bg-white border border-gray-200 rounded-2xl p-6 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
													...
													< div className="flex gap-2 pt-4 border-t border-gray-100" >
														<button
															...
															className="flex-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white py-2 px-4 rounded-lg text-sm font-medium hover:from-blue-700 hover:to-purple-700 transition-all duration-200"
														>
															View Profile
														</button>
